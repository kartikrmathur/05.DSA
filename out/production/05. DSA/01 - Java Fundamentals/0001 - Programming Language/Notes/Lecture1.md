...
Kotlin doesn't have its own memory model when targeting the JVM (which is what you're using for Android/backend) 
— it's identical to Java's:

Variables still live on the stack, objects still live on the heap, and the same JVM garbage collector handles deallocation automatically.
Kotlin gives you no manual memory control either, same as Java.

# A couple of Kotlin-specific nuances worth knowing on top of that, since interviewers sometimes probe this:

1. Primitive boxing: Int, Long, Boolean, etc. in Kotlin compile down to JVM primitives (stack-stored) when possible,
but get auto-boxed into their wrapper objects (heap-stored) when used as nullable types (Int?) or in generics (List<Int>) 
— same boxing behavior as Java's Integer, just more hidden since Kotlin doesn't make you write the wrapper type yourself.

2. data class: still a regular heap-allocated object like any Java class — 
the auto-generated equals()/hashCode()/toString() don't change where it lives in memory.
Kotlin/Native (used for iOS/desktop targets, not Android/JVM) has a genuinely different memory manager (its own ARC-style system), but that's irrelevant to you as an Android/backend Java+Kotlin dev — not worth mentioning in an interview unless they specifically ask about multiplatform.
